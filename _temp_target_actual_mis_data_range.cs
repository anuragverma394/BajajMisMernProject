public JsonResult TargetActualMISData(string F_Name, string Date, string Todate)
        {
            string CDATE = ""; Int32 SRNO = 0; string DATE = ""; string FACTORY = "";
            string CCODE = ""; string CNAME = ""; decimal PQTY = 0; decimal RQTY = 0; decimal CLBAL = 0; decimal TOTRQTY = 0; decimal DRQTY = 0; decimal PERC = 0;
            List<TargetActualMISReport> ActualMISPeriod = new List<TargetActualMISReport>();
            try
            {
                //if (F_Name!="0")
                //{  TM_BH_ONDATEQTY,TM_BH_TODATEQTY,TM_CH_ONDATEQTY,TM_CH_TODATEQTY,TM_BAGASSE_ONDATEQTY,TM_BAGASSE_TODATEQTY
                ...
                dt1.Columns.AddRange(new DataColumn[57] {
                            ...
                            //new DataColumn("ALHL_SYRUP_TARGET",typeof(string)),
                            //new DataColumn("ALHL_SYRUP_ONDATE",typeof(string)),
                            //new DataColumn("ALHL_SYRUP_TODATE",typeof(string)),
                            //new DataColumn("ALHL_BH_TARGET",typeof(string)),
                            //new DataColumn("ALHL_BH_ONDATE",typeof(string)),
                            //new DataColumn("ALHL_BH_TODATE",typeof(string)),
                            //new DataColumn("ALHL_CH_TARGET",typeof(string)),
                            //new DataColumn("ALHL_CH_ONDATE",typeof(string)),
                            //new DataColumn("ALHL_CH_TODATE",typeof(string)),
                ...
                if (Date.Length > 0)
                {
                    ...
                    dates = DateTime.ParseExact(Date.Replace("-", "/"), "dd/MM/yyyy", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd");
                    todate = DateTime.ParseExact(Todate.Replace("-", "/"), "dd/MM/yyyy", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd");
                }
                string f_code = "";
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    for (int a = 0; a < dt.Rows.Count; a++)
                    {
                        ...
                        // dr1["ALHL_SYRUP_TARGET"] = dt.Rows[a]["CP_Alcohol_Syrup"].ToString();
                        // dr1["ALHL_BH_TARGET"] = dt.Rows[a]["CP_Alcohol_BH"].ToString();
                        //dr1["ALHL_CH_TARGET"] = dt.Rows[a]["CP_Alcohol_CH"].ToString();
                        ...
                        DataTable dtact = OBJTARGET.GEActualData(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        ...
                        DataTable Pdt = OBJTARGET.GetPolPercTodate(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        if (Pdt.Rows.Count > 0)
                        {
                        ...
                        DataTable PPdt = OBJTARGET.CalculateProduction(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        if (PPdt.Rows.Count > 0)
                        {
                            if (Convert.ToDecimal(PPdt.Rows[0]["ProductionOnDate"].ToString()) > 0)
                            {
                                dr1["SG_PROD_ONDATE"] = PPdt.Rows[0]["ProductionOnDate"].ToString();
                            }
                            else
                            {
                                dr1["SG_PROD_ONDATE"] = 0;
                            }
                            if (Convert.ToDecimal(PPdt.Rows[0]["ProductionToDate"].ToString()) > 0)
                            {
                                dr1["SG_PROD_TODATE"] = PPdt.Rows[0]["ProductionToDate"].ToString();
                            }
                            else
                            {
                                dr1["SG_PROD_TODATE"] = 0;
                            }
                            if (Convert.ToDecimal(PPdt.Rows[0]["BagasseQtyOnDate"].ToString()) > 0 || Convert.ToDecimal(PPdt.Rows[0]["BagasseQtyOnDate"].ToString()) < 0)
                            {
                                dr1["BAGASE_QTY_ONDATE"] = Convert.ToDecimal(PPdt.Rows[0]["BagasseQtyOnDate"].ToString());
                            }
                            else
                            {
                                dr1["BAGASE_QTY_ONDATE"] = 0;
                            }
                            if (Convert.ToDecimal(PPdt.Rows[0]["BagasseQtyToDate"].ToString()) > 0 || Convert.ToDecimal(PPdt.Rows[0]["BagasseQtyToDate"].ToString()) < 0)
                            {
                                dr1["BAGASE_QTY_TODATE"] = Convert.ToDecimal(PPdt.Rows[0]["BagasseQtyToDate"].ToString());
                            }
                            else
                            {
                                dr1["BAGASE_QTY_TODATE"] = 0;
                            }
                            if (Convert.ToDecimal(PPdt.Rows[0]["PowerProducedKwhOnDate"].ToString()) > 0)
                            {
                                dr1["POWER_PRODUCED_ONDATE"] = Convert.ToDecimal(PPdt.Rows[0]["PowerProducedKwhOnDate"].ToString());
                            }
                            else
                            {
                                dr1["POWER_PRODUCED_ONDATE"] = 0;
                            }
                            if (Convert.ToDecimal(PPdt.Rows[0]["PowerProducedKwhToDate"].ToString()) > 0)
                            {
                                dr1["POWER_PRODUCED_TODATE"] = Convert.ToDecimal(PPdt.Rows[0]["PowerProducedKwhToDate"].ToString());
                            }
                            else
                            {
                                dr1["POWER_PRODUCED_TODATE"] = 0;
                            ...
                            if (Convert.ToDecimal(PPdt.Rows[0]["PowerExportKwhOnDate"].ToString()) > 0)
                            {
                                dr1["POWER_EXPORT_ONDATE"] = Convert.ToDecimal(PPdt.Rows[0]["PowerExportKwhOnDate"].ToString());
                            }
                            else
                            {
                                dr1["POWER_EXPORT_ONDATE"] = 0;
                            }
                            if (Convert.ToDecimal(PPdt.Rows[0]["PowerExportKwhToDate"].ToString()) > 0)
                            {
                                dr1["POWER_EXPORT_TODATE"] = Convert.ToDecimal(PPdt.Rows[0]["PowerExportKwhToDate"].ToString());
                            }
                            else
                            {
                                dr1["POWER_EXPORT_TODATE"] = 0;
                        ...
                        DataTable Ldt = OBJTARGET.GetLossMolPercOndateTodateData(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        if (Ldt.Rows.Count > 0)
                        {
                        ...
                        //DataTable dtBhCh = OBJTARGET.GetBHCHPercAndQtyOndateTodateData(dt.Rows[a]["F_Code"].ToString(), dates);
                        DataTable dtBhCh = OBJTARGET.GetBHCHPercOndateTodate(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        if (dtBhCh.Rows.Count > 0)
                        {
                            for (int i = 0; i < dtBhCh.Rows.Count; i++)
                            {
                                ...
                                if (dtBhCh.Rows[i]["type"].ToString() == "2")
                                {
                                    if (Convert.ToDecimal(dtBhCh.Rows[i]["OnDatePerc"].ToString()) > 0)
                                    {
                        ...
                        DataTable dtBhChQty = OBJTARGET.GetBHCHQtlsOndateTodate(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        ...
                        //new code alcohol
                        DataTable dtAL = OBJTARGET.GetAlcohoOndateTodateData(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        ...
                        DataTable CrushDt = OBJTARGET.GetOndateTodateCruch(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        ...
                        DataTable Tdt = OBJTARGET.GetBhChCaneCrush(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                        if (Tdt.Rows.Count > 0)
                        {
                        ...
                        //LMOS B+CHY PERC
                        DataTable LCrushDt = OBJTARGET.GetOndateTodateCruch(dt.Rows[a]["F_Code"].ToString(), dates, todate);
                ...
                foreach (DataRow dr in dt1.Rows)
                {
                    ActualMISPeriod.Add(
                        new TargetActualMISReport
                        {
                            ...
                            //ALHL_SYRUP_TARGET = Convert.ToString(dr["ALHL_SYRUP_TARGET"]),
                           // ALHL_SYRUP_ONDATE = Convert.ToString(dr["ALHL_SYRUP_ONDATE"]),
                            ///ALHL_SYRUP_TODATE = Convert.ToString(dr["ALHL_SYRUP_TODATE"]),
                            ///ALHL_BH_TARGET = Convert.ToString(dr["ALHL_BH_TARGET"]),
                            //ALHL_BH_ONDATE = Convert.ToString(dr["ALHL_BH_ONDATE"]),
                            //ALHL_BH_TODATE = Convert.ToString(dr["ALHL_BH_TODATE"]),
                            //ALHL_CH_TARGET = Convert.ToString(dr["ALHL_CH_TARGET"]),
                            //ALHL_CH_ONDATE = Convert.ToString(dr["ALHL_CH_ONDATE"]),
                            ///ALHL_CH_TODATE = Convert.ToString(dr["ALHL_CH_TODATE"]),
            ...
            return Json(ActualMISPeriod,JsonRequestBehavior.AllowGet);
        }
