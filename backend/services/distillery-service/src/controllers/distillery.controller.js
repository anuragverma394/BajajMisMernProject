...
exports.Index = async (req, res) => {
  return res.status(200).json({ success: true, message: `${CONTROLLER} service ready`, data: null });
};

exports.CHeavyEthanolReport = async (req, res, next) => {
  try {
...
exports.BHeavyEthanolReport = async (req, res, next) => {
  try {
    const parsed = validation.parseReport(req);
    if (!parsed.ok) {
      return res.status(400).json({ success: false, message: parsed.message, data: null });
...
exports.SyrupEthanolReport = async (req, res, next) => {
  try {
    const parsed = validation.parseReport(req);
    if (!parsed.ok) {
      return res.status(400).json({ success: false, message: parsed.message, data: null });
