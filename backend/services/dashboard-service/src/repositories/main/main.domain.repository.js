...
const receiptTokenDateColumnCache = new Map();
async function resolveReceiptTokenDateColumn(season) {
  const key = String(season || '');
  if (receiptTokenDateColumnCache.has(key)) {
    return receiptTokenDateColumnCache.get(key);
  }
  let picked = 'TT_DATE';
  try {
    const cols = await getTableColumnList(season, 'Receipt');
    const candidate = pickExistingColumnFromList(cols, [
      'tt_TokDatetime',
      'TT_TokDatetime',
      'tt_tokdatetime',
      'TT_DATE',
  ...
  receiptTokenDateColumnCache.set(key, picked);
  return picked;
}
...
exports.Units = async (req, res, next) => {
  try {
    ...
    // Fallback for environments where MI_Factory is empty but Factory is populated.
    if (!rows.length) {
      rows = await executeQuery(
        `SELECT f_code AS f_Code,
                CONCAT(f_name, ' (', ISNULL(F_SHORT, ''), ')') AS F_Name,
                f_name,
                ISNULL(F_SHORT, '') AS F_Short
         FROM Factory
         ORDER BY f_name ASC`,
        {},
        season
...
exports.HomeFact = async (req, res, next) => {
  ...
  try {
    ...
    if (mode === 'survey') {
      const surveySource = await resolveSurveyAggregateSource(season);
      if (!surveySource) {
    ...
    const applyUserFactoryFilter = !codes && await hasUserFactoryAccess(season, userId);
    if (F_Code && !codes) {
      return res.status(400).json({
        success: false,
    ...
    const factoryFilter = codes
      ? buildParametrizedInClause(factoryColumn, codes, factoryParams, 'hf')
    ...
    if (effectiveRange.from === effectiveRange.to && effectiveMode !== 'survey') {
      let hourlyData = await fetchHomeFactHourly({
        season,
        codes,
        userId,
      ...
      if (!hourlyData?.MyList?.length && applyUserFactoryFilter && !codes) {
        console.log('[Dashboard][HomeFact] retry hourly without user factory filter');
        hourlyData = await fetchHomeFactHourly({
          season,
          codes,
          userId,
          dateIso: effectiveRange.from,
      ...
      if (!hourlyData?.MyList?.length) {
        console.log('[Dashboard][HomeFact] empty hourly result', {
          mode: effectiveMode,
    ...
    let rows = await fetchHomeFactRowsAdaptive({
      season,
      factoryFilter,
      fromDate: effectiveRange.from,
      toDate: effectiveRange.to,
    ...
    if (!rows.length && applyUserFactoryFilter && !codes) {
      console.log('[Dashboard][HomeFact] retry without user factory filter');
      rows = await fetchHomeFactRowsAdaptive({
        season,
        factoryFilter: '',
        fromDate: effectiveRange.from,
        toDate: effectiveRange.to,
    ...
    if (!rows.length && !codes) {
      const latestRange = await resolveLatestDashboardRange(season, 15);
      if (latestRange && (latestRange.from !== effectiveRange.from || latestRange.to !== effectiveRange.to)) {
        console.log('[Dashboard][HomeFact] fallback to latest available range', {
    ...
    if (!rows.length) {
      console.log('[Dashboard][HomeFact] empty result', {
        mode: effectiveMode,
        fromDate: effectiveRange.from,
        toDate: effectiveRange.to,
        F_Code: codes || null,
    ...
    console.log('[Dashboard][HomeFact] error', {
      message: error?.message,
      stack: error?.stack
    });
...
exports.OverShootForCenters = async (req, res) => {
  try {
    ...
    const applyUserFactoryFilter = !codes && await hasUserFactoryAccess(season, userId);
    const receiptTokenDateCol = await resolveReceiptTokenDateColumn(season);
    const tokenDateExpr = `r.${quoteSqlIdentifier(receiptTokenDateCol)}`;
    let activeRange = { ...range };
    const params = { fromDate: activeRange.from, toDate: activeRange.to, userId };
    const factoryFilter = codes
    ...
    const buildOverShootSql = (activeFactoryFilter) => `WITH cte AS (
         SELECT
           ...
           DATEDIFF(
             ...
             cf.Ch_dep_date,
             CASE
               WHEN ISNULL(${tokenDateExpr}, '1900-01-01') = '1900-01-01'
                 THEN ISNULL(tk.TT_CRDATE, cf.Ch_dep_date)
               ELSE ${tokenDateExpr}
             END
           ) AS TravelMinutes
         FROM challan_final cf WITH (NOLOCK)
         LEFT JOIN Receipt r WITH (NOLOCK)
           ON r.tt_factory = cf.ch_Factory
          AND r.tt_center = cf.Ch_Centre
          AND r.tt_chalanNo = cf.ch_challan
         LEFT JOIN T_TOKEN tk WITH (NOLOCK)
           ON tk.TT_FACTORY = cf.ch_factory
          AND tk.TT_CHLN = cf.ch_challan
         WHERE cf.Ch_Cancel = 0
           AND CAST(cf.Ch_dep_date AS date) BETWEEN @fromDate AND @toDate
           ${activeFactoryFilter}
       )
       SELECT
         c.ch_Factory,
         ...
         SUM(ISNULL(c.TravelMinutes, 0)) AS TotalMinutes,
         CONVERT(varchar(5), SUM(ISNULL(c.TravelMinutes, 0)) / 60) + ':' + CONVERT(char(2), SUM(ISNULL(c.TravelMinutes, 0)) % 60) AS TotalTime,
         (SUM(ISNULL(c.TravelMinutes, 0)) / NULLIF(COUNT(c.C_Code), 0)) AS avgmint,
         CAST(CONVERT(varchar(5), (SUM(ISNULL(c.TravelMinutes, 0)) / NULLIF(COUNT(c.C_Code), 0)) / 60)
           + ':' + CONVERT(char(2), (SUM(ISNULL(c.TravelMinutes, 0)) / NULLIF(COUNT(c.C_Code), 0)) % 60) AS varchar) AS avgtime,
         ...
         (SUM(ISNULL(c.TravelMinutes, 0)) / NULLIF(COUNT(c.C_Code), 0))
       ...
       LEFT JOIN Centre cnt WITH (NOLOCK)
         ON cnt.c_factory = c.ch_Factory
        AND cnt.c_code = c.C_Code
       JOIN MI_Factory mfac WITH (NOLOCK)
         ON c.ch_Factory = mfac.F_Code
       ${groupBy}`;

    let rows = await executeQuery(
      buildOverShootSql(factoryFilter),
      params,
      season,
      { timeoutMs: 120000 }
    );

    let filtered = rows.filter((item) => Number(item.diff || 0) > 0);
    if (!filtered.length && applyUserFactoryFilter && !codes) {
      console.log('[Dashboard][OverShootForCenters] retry without user factory filter');
      rows = await executeQuery(
        buildOverShootSql(''),
        params,
        season,
      ...
      filtered = rows.filter((item) => Number(item.diff || 0) > 0);
    }
    if (!filtered.length && !codes) {
      const latestRange = await resolveLatestDashboardRange(season, 15);
      if (latestRange && (latestRange.from !== activeRange.from || latestRange.to !== activeRange.to)) {
        console.log('[Dashboard][OverShootForCenters] fallback to latest available range', {
          fromDate: latestRange.from,
          toDate: latestRange.to
        });
        params.fromDate = latestRange.from;
        params.toDate = latestRange.to;
        activeRange = latestRange;
        rows = await executeQuery(
          buildOverShootSql(''),
          params,
          season,
    ...
    if (!filtered.length) {
      console.log('[Dashboard][OverShootForCenters] empty result', {
    ...
    console.log('[Dashboard][OverShootForCenters] error', {
      message: error?.message,
      stack: error?.stack
    });
    logControllerError('OverShootForCenters', req, error, {
      F_Code: req.body?.F_Code || req.query?.F_Code || null,
      txtdaterange: req.body?.txtdaterange || req.query?.txtdaterange || null
    });
...
exports.TokenGrossTare = async (req, res) => {
  try {
    ...
    const buildMultiFactoryQuery = (factoryPredicate) => `
        ...
        FROM (
          ...
          FROM (
            SELECT COALESCE(RTRIM(mf.F_Name), RTRIM(fa.F_Name), CAST(tt_factory AS varchar(10))) AS F_Name,
                   r.tt_factory AS F_Code,
                   DATEDIFF(MINUTE, ${tokenDateExpr}, r.tt_GroDateTime) AS TGM,
                   DATEDIFF(MINUTE, r.tt_GroDateTime, r.TT_TARE_DT) AS GTrM,
                   DATEDIFF(MINUTE, ${tokenDateExpr}, r.TT_TARE_DT) AS TTM
            FROM Receipt r
            LEFT JOIN MI_Factory mf ON mf.F_code = r.tt_factory
            LEFT JOIN Factory fa ON fa.F_code = r.tt_factory
            WHERE CAST(r.TT_TARE_DT AS date) BETWEEN @fromDate AND @toDate
              ${factoryPredicate}
          ) X
    ...
    if (singleFactory) {
        ...
        FROM (
          ...
          FROM (
            SELECT c_code, c_name,
                   ...
                   DATEDIFF(MINUTE, ${tokenDateExpr}, r.TT_TARE_DT) AS TTM
            FROM Receipt r
            JOIN Centre f ON f.c_code = r.tt_center
            WHERE r.tt_factory = @singleFactory
              AND CAST(r.TT_TARE_DT AS date) BETWEEN @fromDate AND @toDate
          ) X
          WHERE TGM > 0 AND GTrM > 0 AND TTM > 0
            AND ((CASE WHEN TGM > 120 THEN TGM - 120 ELSE 0 END) > 0
              OR (CASE WHEN GTrM > 120 THEN GTrM - 120 ELSE 0 END) > 0
      ...
      query = buildMultiFactoryQuery(factoryPredicate);
    }

    let rows = await executeQuery(query, params, season, { timeoutMs: 120000 });
    if (!rows.length && applyUserFactoryFilter && !codes && !singleFactory) {
      console.log('[Dashboard][TokenGrossTare] retry without user factory filter');
      rows = await executeQuery(buildMultiFactoryQuery(''), params, season, { timeoutMs: 120000 });
    }
    if (!rows.length && !codes && !singleFactory) {
      const latestRange = await resolveLatestDashboardRange(season, 15);
      if (latestRange && (latestRange.from !== activeRange.from || latestRange.to !== activeRange.to)) {
        console.log('[Dashboard][TokenGrossTare] fallback to latest available range', {
          fromDate: latestRange.from,
          toDate: latestRange.to
        });
        params.fromDate = latestRange.from;
    ...
    if (!rows.length) {
      console.log('[Dashboard][TokenGrossTare] empty result', {
        fromDate: activeRange.from,
        toDate: activeRange.to,
        F_Code: codes || null,
        userId: userId || null,
    ...
    console.log('[Dashboard][TokenGrossTare] error', {
      message: error?.message,
      stack: error?.stack
    });
    logControllerError('TokenGrossTare', req, error, {
      F_Code: req.body?.F_Code || req.query?.F_Code || null,
      txtdaterange: req.body?.txtdaterange || req.query?.txtdaterange || null
    });
    return res.status(500).json({ success: false, message: error.message });
...
exports.SWRD = async (req, res, next) => {
  try {
    ...
    const buildUnitsWhere = (codeColumn) => {
      let where = '';
      if (codes) {
        where += buildParametrizedInClause(codeColumn, codes, currentParams, 'swu');
      } else if (!isAdmin && userId) {
        where += ` AND ${codeColumn} IN (SELECT FactID FROM MI_UserFact WITH (NOLOCK) WHERE UserID = @userId)`;
      }
      return where;
    };

    const runSummaryQuery = async (tableName, codeColumn, nameColumn) => executeQuery(
      `WITH units AS (
         SELECT ${codeColumn} AS Cn_Unit, RTRIM(${nameColumn}) AS Unit
         FROM ${tableName} f WITH (NOLOCK)
         WHERE 1=1 ${buildUnitsWhere(`f.${codeColumn.split('.').pop()}`)}
       )
       SELECT u.Cn_Unit,
              u.Unit,
    ...
    let currentRows = await runSummaryQuery('MI_Factory', 'f.F_Code', 'f.F_Name').catch(() => []);
    if (!currentRows.length) {
      currentRows = await runSummaryQuery('Factory', 'f.f_code', 'f.f_name').catch(() => []);
    }
    ...
    const data = rows.length
      ...
      : normalizedParameters.map((p) => {
          ...
          return {
            ...
            Id: Number(p.Id || p.id || 0),
            Pm_Name: String(p.Pm_Name || p.name || ''),
            name: String(p.Pm_Name || p.name || ''),
            current: 0,
            previous: 0,
            Tpm_Unit: Number(unit || 0),
            F_Name: '',
            Tpm_DATE: dmy
          };
        });
