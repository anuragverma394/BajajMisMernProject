...
const cache = require('../../../../shared/utils/cache.service');
...
function buildFactoryInClause(codes, tableAlias = '') {
  if (!codes) return '';
  const prefix = tableAlias ? `${tableAlias}.` : '';
  return ` AND ${prefix}F_Code IN (${codes})`;
}
...
async function resolveCaneCrushTable(season) {
  ...
  const row = probe[0] || {};
  if (row.CaneCrushObj) return 'CaneCrush';
  if (row.MiCaneCrushObj) return 'MI_CaneCrush_Entry';
  return 'CaneCrush';
}
...
exports.SugarWhatsAppReportView = async (req, res, next) => {
  try {
    ...
    const tableName = await resolveCaneCrushTable(season);
...
exports.DailyRainfallData = async (req, res, next) => {
  try {
    const season = req.user?.season;
    const tableName = await resolveRainfallTable(season);
    const tableSql = quoteSqlObjectName(tableName);
...
exports.MonthlyEntryReportView = async (req, res, next) => {
  try {
    ...
    const parameterTableName = await resolveMonthlyParameterTable(season);
    ...
    const entryTableName = await resolveMonthlyEntryTable(season);
    if (!entryTableName) {
...
exports.MonthlyEntryReport = async (req, res, next) => {
  try {
    ...
    const season = req.user?.season;
    const tableName = await resolveMonthlyParameterTable(season);
    const tableSql = quoteSqlObjectName(tableName);
    const rows = await executeQuery(
...
exports.MonthlyEntryReport_2 = async (req, res, next) => {
  try {
    ...
    const command = String(req.body.Command || req.body.command || '').trim().toLowerCase();
    const id = String(req.body.id || '').trim();
    const tableName = await resolveMonthlyEntryTable(season);
    if (!tableName) {
      return res.status(200).json({
        success: true,
