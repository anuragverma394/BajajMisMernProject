...
async function getPool(seasonValue) {
  ...
  const hasOdbcConn = conn && /Driver=\{.+\}/i.test(conn);
  if (conn && hasOdbcConn && !sqlNative) {
    throw new Error('SQL_CONN_2526 uses an ODBC connection string but msnodesqlv8 is not available');
  }

  const driver = conn ? (hasOdbcConn && sqlNative ? sqlNative : sql) : dbConfig.driver;
  const config = conn
    ? {
