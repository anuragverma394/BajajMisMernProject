...
const Report_TargetActualMISPeriodReport = () => {
    ...
    const [filters, setFilters] = useState({
        F_Code: '',
        FromDate: formatTodayDmy(),
        ToDate: formatTodayDmy()
    });
    ...
    const handleSearch = async () => {
        const from = toDmy(filters.FromDate);
        const to = toDmy(filters.ToDate);
        if (!from || !to) {
        ...
        try {
            ...
            const response = await reportService.getTargetActualMISPeriodReport(payload);
        ...
        <div className="min-h-screen bg-slate-50 p-4">
            ...
            <div className="rounded-lg border border-emerald-200 bg-white shadow-sm">
                <div className="rounded-t-lg bg-emerald-600 px-4 py-3 text-sm font-semibold text-white">
                    Target VS Actual Report (Period)
                </div>
                <div className="space-y-4 p-4">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        <div>
                            <label className="mb-2 block text-xs font-semibold text-slate-700">From Date</label>
                            <input
                                name="FromDate"
                        ...
                        <div>
                            <label className="mb-2 block text-xs font-semibold text-slate-700">To Date</label>
                            <input
                                name="ToDate"
                                ...
                                {rows.length === 0 ? (
                                    <tr>
                                        <td colSpan={1 + 12 + metricGroups.length * 3} className="border border-emerald-100 px-4 py-10 text-center text-slate-400">
                                            No data found. Select a factory and date range, then click Search.
                                        </td>
                                    </tr>
                                ) : (
                                    rows.map((row, idx) => {
...
export default Report_TargetActualMISPeriodReport;
