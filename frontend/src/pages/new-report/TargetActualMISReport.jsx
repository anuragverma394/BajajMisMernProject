...
const NewReport_TargetActualMISReport = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const response = await reportNewService.getTargetActualMISDataMis(payload);
            const data = Array.isArray(response)
                ? response
                : Array.isArray(response?.data)
                    ? response.data
                    : response?.data?.data || response?.result || [];
            setRows(data);
            if (!data.length) {
...
export default NewReport_TargetActualMISReport;
